...
function preload() {
  onimage = loadImage('./data/trump.jpg'); // this is on
  offimage = loadImage('./data/clinton.jpg');
  rigged = loadSound('./data/rigged.mp3');
}
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
     ...
     // write pixels into destination image, scaled to 0 or 255:
      ...
      // draw other stuff:
      if(result==1) // "WHITE PIXEL"
      {
        //fill(255, 0, 0);
        //ellipse(i/howwide*width, j/howtall*height, 15, 15);
        image(onimage, i/howwide*width, j/howtall*height, 25, 25);
        rigged.play
      }
      else // "BLACK PIXEL"
      {
        ...
        image(offimage, i/howwide*width, j/howtall*height, 25, 25);
      }
